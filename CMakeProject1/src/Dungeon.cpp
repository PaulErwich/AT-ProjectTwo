...
void Dungeon::GenerateDungeonLayout()
{
	...
	for (int i = 0; i < MAP_HEIGHT; i++)
	{
		for (int j = 0; j < MAP_WIDTH; j++)
		{
			...
			if (left) rooms.push_back(DOOR1_L);
			if (right) rooms.push_back(DOOR1_R);
			if (top) rooms.push_back(DOOR1_T);
			if (bottom) rooms.push_back(DOOR1_B);
			...
			if ((left && right && top && bottom) ||
				...
				rooms.push_back(DOOR4); // If all sides have access doors, or > 1 side does and other sides are empty
			...
			if (left && R == DOOR_NONE && T == DOOR_NONE && B == DOOR_NONE) rooms.push_back(DOOR1_L);
			if (L == DOOR_NONE && right && T == DOOR_NONE && B == DOOR_NONE) rooms.push_back(DOOR1_R);
			if (L == DOOR_NONE && R == DOOR_NONE && top && B == DOOR_NONE) rooms.push_back(DOOR1_T);
			if (L == DOOR_NONE && R == DOOR_NONE && T == DOOR_NONE && bottom) rooms.push_back(DOOR1_B);
